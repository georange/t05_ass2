...
/* No processing: using this line
	float fcol = 0;
	int dense = 5;
	float sizeH = 1/tex_height;
	float sizeW = 1/tex_width;

	vec2 bl_corner = uv - vec2(sizeW,sizeH) / 2.0f;

	for (int i = 0; i < dense; i++) {
		for (int j = 0; j < dense; j++) {
			fcol += textureOffset(tex, uv, bl_corner + vec2(i*sizeW/(dense-1), j*sizeH/(dense-1)));
		}
	}

	color = fcol/(dense*dense);
*/
